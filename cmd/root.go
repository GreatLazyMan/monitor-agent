...
import (
	...
	"github.com/GreatLazyMan/monitor-agent/cmd/app"
	"github.com/GreatLazyMan/monitor-agent/cmd/options"
...
func run(opt *options.Options) {
	...
	if err != nil {
		klog.Errorf("Failed to notify systemd the message READY=1 %v", err)
	}
	klog.Info("Waiting for all goroutines to exit")
